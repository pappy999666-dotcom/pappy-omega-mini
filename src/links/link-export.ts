...
function renderCard(record: LinkRecord): string {
  ...
  const status = record.metadata?.joinClassification ?? record.bucket;
  ...
  return `<article class="card"><div class="card-head"><div class="title">${escapeHtml(title)}</div><span class="status">${escapeHtml(status)}</span></div><a class="url" href="${escapeHtml(record.canonicalUrl)}">${escapeHtml(record.canonicalUrl)}</a><div class="meta"><div><span class="label">Members</span><span class="value">${escapeHtml(memberCount)}</span></div><div><span class="label">Duplicates</span><span class="value">${escapeHtml(String(record.duplicateCount))}</span></div><div><span class="label">First seen</span><span class="value">${escapeHtml(firstSeen)}</span></div><div><span class="label">Last checked</span><span class="value">${escapeHtml(checked)}</span></div></div></article>`;
}

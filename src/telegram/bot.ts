...
export function createTelegramBot(): Telegraf<Context> {
  ...
  bot.action(/^owner:sudo:(global|omni):(list|add|remove)$/, async (ctx) => {
    ...
    if (action === "list") {
      ...
      return edit(ctx, pageText(`${scope === "global" ? "Global" : "Omni"} Sudo`, infoResponse(`${scope === "global" ? "All User Sessions" : "All Owner Sessions"}`, identities.length ? identities.map((item) => `<code>${escapeHtml(item)}</code>`).join("\n") : "No WhatsApp sudo identities configured.")), keyboard([[btn("＋ Add", `owner:sudo:${scope}:add`, "success"), btn("− Remove", `owner:sudo:${scope}:remove`, "danger")], [btn("↻ Refresh", `owner:sudo:${scope}:list`)], [btn(scope === "global" ? "Open Omni Sudo" : "Open Global Sudo", `owner:sudo:${scope === "global" ? "omni" : "global"}:list`)], [btn("‹ Admin Panel", "admin:panel")]]));
    }

...
| ID | Requirement | Evidence | Status |
|---|---|---|---|
| P1 | Label → phone → persisted session → native pairing code | Pairing wizard and session registry | PARTIAL |
| P2 | Custom configured pairing code with compatibility handling | `requestPairingCode` | PARTIAL |
...
| W5 | Minimum commands: ping, menu, pair, setsudo, setprefix, gstatus, allstatus, stopstatus, allchat, stopchat, tag, stoptag, allstatusx, gstatusx, allchatx | Registry and adapters | PARTIAL |
...
| W9 | Real status/chat/tag operations with bounded queue/cancel/progress/audit | Workers and transport | TO AUDIT |
| W10 | Explicit `.pair` ownership and authorization chain | Command handler | TO AUDIT |

...
Each signed release carries the complete pinned dependency manifest. The current native WhatsApp/link-preview runtime includes pinned `@crysnovax/baileys`, `link-preview-js`, `pino`, and `sharp` dependencies. If a future release adds or changes a dependency, the panel rewrites its private `package.json`, installs the required packages automatically, verifies every declared package version, and retries installation without starting an incomplete runtime. Updates are disabled with `PAPPY_WORKLOAD_AUTO_UPDATE=false` when a panel operator needs a maintenance freeze. No MongoDB URI, Redis URI, Telegram token, Telegram pairing code after registration, or release private key is delivered to the panel.
